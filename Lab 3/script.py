...
# get a specific instance and read values other than position 1 e.g. slice
# using 1:
idx = 21
...
# train_data_file = open("mnist/mnist_train_100.csv", 'r') # open and read
# the 100 instances in the text file
...
# test_data_file = open("mnist/mnist_test_10.csv", 'r') # read the file
# with 10 instances first
...
class neuralNetwork:
    ...
    def __init__(self, inputnodes=784, hiddennodes=200, outputnodes=10, learningrate=0.1, batch_size=1, epochs=10):
        ...
        # size is set to the dimensions of the number of hnodes, inodes and
        # onodes
        ...
        # lambda is a short cut function which is executed there and then with
        # no def (i.e. like an anonymous function)
    ...
    # note the first part is very similar to the query function because they
    # both require the forward pass
        ...
        for e in range(self.ep):
            ...
            for batch in self.batch_input(train_inputs):
                ...
                for instance in batch:
                    ...
                    inputs = (np.asfarray(
                        all_values[1:]) / 255.0 * 0.99) + 0.01
                    # create the target output values for each instance so that we can use it with the neural net
                    # note we need 10 nodes where each represents one of the
                    # digits
                    # all initialised to 0.01
                    ...
                    # to calculate the error we need to compute the element
                    # wise diff between target and actual
                    output_errors = targets - final_outputs
                    # Next distribute the error to the hidden layer such that hidden layer error
                    # is the output_errors, split by weights, recombined at
                    # hidden nodes
                    ...
                    # delta_wih are the gradients between input and hidden
                    # weights
                    ...
                    # this is the sum of squared error accumilated over each
                    # batced instance
                ...
                # obviously if batch size = 1 then we dont need to bother with
                # an average
    ...
    def query(self, inputs_list):
        ...
        # propogate input into hidden layer. This is the start of the forward
        # pass
        hidden_inputs = np.dot(self.wih, inputs)

        # squash the content in the hidden node using the sigmoid function
        # (value between 1, -1)
        hidden_outputs = self.activation_function(hidden_inputs)

        # propagate into output layer and the apply the squashing sigmoid
        # function
        final_inputs = np.dot(self.who, hidden_outputs)
    ...
    def test(self, test_inputs):
        ...
        for instance in test_inputs:
            # extract the input feature values for the instance
            all_values = instance.split(',')

            # get the target class for the instance
            target_label = int(all_values[0])

            # scale and shift the inputs this is to make sure values dont lead
            # to zero when multiplied with weights
            ...
            # get the index of the highest output node as this corresponds to
            # the predicted class
...
print("Percentage of training data used:",
      (len(mini_training_data) / len(train_data_list)) * 100)
...
# iteratre through each tested instance and accumilate number of correct
# predictions
...
n_list = []
batch_sizes = [1, 20, 100, 200, 500, len(mini_training_data)]
# lets try two different configurations of hidden node layer
hidden_nodes = [200, 5]
